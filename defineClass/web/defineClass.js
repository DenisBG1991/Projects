 ...
 *              Если определено, это имя сохраняется в свойстве classname объекта-прототипа.
 *
 ...
 *
 * constuctor:  Функция-конструктор класса. В случае отсутствия будет использована новая
 *              пустая функция. Это значение станет возвращаемым значением функции,
 *              а также сохранится в свойстве cunstructor объекта-прототипа.
 *
 ...
 *              может рассматриваться (с использованием методов грубого определения типа)
 *              как экземпляр указанных типов. Если данное свойство не определено,
 *              проверка выполняться не будет.
 **/
function defineClass (data){
    // Извлечь значение полей из объекта-аргумента.
    ...
    // Создать объект, который станетпрототипом класса.
    var proto = new superclass();

    // Удалить все неунаследуемые свойства из нового объекта-прототипа.
    for (var p in proto){
    ...
    for (var i = 0; i < borrows.length; i++){
        ...
        borrows[i] = c;
        // Скопировать методы из прототипа объекта в наш прототип.
    ...
    // Эта операция может переопределить меоды, скопированные из классов-смесей.
    ...
    for (var i = 0; i < provides.length; i++){ // для каждого класса
        ...
        for (var p in c.prototype){            // для каждого свойства
            ...
            throw alert("Кдасс " + classname + " не предоставляем метод " +
                c.classname + "." + p);
        }
    }
