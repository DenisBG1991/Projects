 ...
 *
 * В этом модуле определяется единственная функция maketoc(), тыкже он
 * решистрирует обработчик событий onload, благодаря чему функция
 * запускается автоматически сразу же после загрузки документа.
 * После запуска функция maketoc() вначале просматривает документ в поисках
 ...
 * maketoc() обходит документ, отыскивая все теги от <h1> до <h6>
 ...
 * Номера разделов вставляются в загловки, принадлежащие классу "TOCSectNum",
 ...
 * Чтобы изменить этот тект (например, с целью интернационализауии),
...
function maketoc(){
    ...
    for (var s = 0; s < sections.length; s++){
        ...
        // и сбросить в ноль номера всех нижелижащих подуровней
        sectionNumbers[level-1]++;
        for (var j = level; j < 6; j++) sectionNumbers[j]=0;
    ...
    function findSections(n, sects){
        ...
        for (var m = n.firstChild; m != null; m = m.nextSibling){
            // Прорустить узлы, которые не являются элементами.
