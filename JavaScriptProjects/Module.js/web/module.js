/**
 * Module.js: Функция для работы с модулями и пространствами имен.
 *
...
if (Module && (typeof Module != "object" || Module.NAME))
    throw alert("Пространство имен 'Module' уже существует.");
 ...
 * Если выл задан аргумент version, устанавливает свойство пространство имен VERSION/
 *
 * Отображение нового пространства имен добавляется в объект Module.modules.
 */
Module.createNamespace = function(name, version){
    // Проверка корректности имени. Оно должно существовать и не должно
    // начинаться или заканчиваться символом точки или содержать в строке
    // два символа точки подряд.
    if (!name) throw alert("Module.createNamespace(): не указано имя.");
    if (name.charAt(0) == '.' ||
    name.charAt(name.length-1) == '.' ||
    name.indexOf("..") != -1)
        throw alert("Module.createNamespace: неверное имя " + name);

    // Разбит имя по символам точки и создать иерархию объектов
    ...
    for (var i = 0; i < parts.length; i++){
        ...
        if (!container[part]) container[part] = {};
            ...
            throw alert(n + " уже существует, но не является объектом.");
        }
    ...
    if (namespace.NAME) throw alert("Модуль " + name + " уже определен.");

    // Инициализация поля с именем и версией пространства имен.
    namespace.NAME = name;
    if (version) namespace.VERSION = version;

    // Зарегестрировать это пространство имен в списке модулей.
 ...
 * фатальной ошибки, если модуль, требуемый вышему программному коду, отсутствует.
 */
Module.require = function(name, version){
    if (!(name in Module.modules)){
        throw alert("Модуль " + name + " не определен.");
    }
    ...
    if (!n.VERSION || n.VERSION < version)
        throw alert("Модуль " + name + " имеет версию " +
 ...
 * импорт производится а глобальное пространство имен, однако с помощью
 ...
 * передаваться в виде аргументов вслед за именами модуля и именами
...
Module.importSymbols = function(from){
    ...
    if (!from || typeof from != "object")
        throw alert("Module.importSymbols(): " + "требуется указать объект пространства имен.");
    ...
    // определяемых модулем по умолчанию, или просто импортировать все символы.
    if (symbols.length == 0){
    ...
    // Если пространство имен определяет массив EXPORT и/или EXPORT_OK,
    ...
    if (from.EXPORT || from.EXPORT_OK){
        ...
        // Это даст возможность более эффекивно проверить допустимость импорта символа.
        if (from.EXPORT){
            for (var i = 0; i < from.EXPORT.length; i++){
        ...
        if (from.EXPORT_OK){
            for (var j = 0; j < from.EXPORT_OK.length; j++){
                allowed[from.EXPORT_OK[j]] = true;
            }
        }
    }
    ...
    for (var l = 0; l < symbols.length; l++){
        var s = symbols[l];                 // Имя импортируемого символа.
        if (!(s in from))                   // проверить его наличие.
            alert("Module.importSymbols(): символ " + s + " не определен.");
        if (allowed && !(s in allowed))     // Убедиться, что это общедоступный символ
            alert("Module.importSymbols(): символ " + s +
...
Module.registerInitializationFunction = function(f){
    ...
    // Если обработчик событий onload еще не зарегистрирован, сделать это сейчас.
    Module._registerEventHandler();
};

// Данная функция вызывает зарегистрированные функции инициализации.
// В клиентском JavaScript она автоматически вызывается по окончанию загрузки документа.
// В других контекстах исполнения может потребоваться вызвать эту функцию явно.
Module.runInitializationFunction = function(){
    ...
    // Уничтожить массив, как как такие функции вызываются всего один раз.
...
// как обработчик событий onload, чтобы иметь возможность запустить все функции
// иниуиализации по окончанию загрузки всех модулей.
...
Module._registerEventHandler = function(){
    ...
    if (clientside){
        if (window.addEventListener){     // Регистрация по стандарту W3C DOM.
            window.addEventListener("load", Module.runInitializationFunction, false);

        }
        else if (window.attachEvent){     // Регистрация в IE5+
            window.attachEvent("onload", Module.runInitializationFunction);
        }
        else {
            ...
            window.onload = Module.runInitializationFunction;
        }
    }
